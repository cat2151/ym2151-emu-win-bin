...
Pythonプロジェクトで当リポジトリのライブラリを使用している場合：
...
**A**: すべての言語で新しいライブラリ名に変更が必要です。APIは変更されていないため、ライブラリファイル名の参照のみを更新してください。
...
2. **リンク時のライブラリ名のみ変更**: `-lym2151` → `-lnukedopm` (Rust/Go)、`ym2151.dll` → `nukedopm.dll` (Python)
